****  	Manuscript: Measuring stocks of community wealth and their association with food systems
****	 efforts in rural and urban places
****	Code developed by:  
...
****	For ease of exposition, this code includes the final model specifications only (OLS & SDM) 
****     and excludes all of the robustness checks and model form testing discussed in the paper.
...
** Change directory to folder with data files **
  import excel "C:\Users\tms1\Documents\LocalFoods\GrowNYC\Capital Indecies paper\FullDataFile061521.xlsx", sheet("Data") firstrow
...
*create interactions of capital indexes and nonmetro and nonmetro NonMetro Adjacent (NMA)
...
** Results in Table 5, columns 2 through 4 

eststo: reg d2c_total $caps $caps_nm $caps_nm_NMA drucc1-drucc8 dstate1-dstate47

*** tests for the joint significance of the coefficients of (respectively):
*** Interactions of capital stocks and non-metro indicator
test $caps_nm

*** Interactions of capital stocks and non-metro non-metro adjacent indicator
test $caps_nm_NMA

***  Interactions of capital stocks non-metro & non-metro non-metro adjacent 
test $caps_nm $caps_nm_NMA

** Results in table 5, columns 5 and 6  
...
***  In order to run a Spatial model, the matrix of spatial weights must be square
**   we need to drop all observations that are not used in the regression 

reg d2c_total $caps $caps_nm $caps_nm_NMA drucc1-drucc8 dstate1-dstate48 
predict errors, residuals 
drop if errors==.

*** NOTE: 	different estimation routines require different syntax to obtain objects to be used as W 
...
** NOTE: Because we only use ~2.6k observations, W matrix with "short" max distance of influence result in "islands" observations 
...
**** Spatial Durbin model - ML Table 6 
...
***  capital stocks interacted with non-metro and non-metro non-metro adjacent indicators
test $caps_nm $caps_nm_NMA

*** spatial lags of capital stocks  
test wx_pc1b wx_pc2b wx_pc1c wx_pc2c wx_pc1f wx_pc1h  wx_pc2h wx_pc1n wx_pc2n wx_pc1s wx_pc2s 

*** spatial lags of capital stocks interacted with non-metro indicator
...
*** spatial lags of capital stocks interacted with non-metro non-metro adjacent indicator
...
*** spatial lags of capital stocks interacted with non-metro and non-metro non-metro adjacent indicators 
test wx_pc1b_nm wx_pc2b_nm wx_pc1c_nm wx_pc2c_nm wx_pc1f_nm wx_pc1h_nm  wx_pc2h_nm wx_pc1n_nm wx_pc2n_nm wx_pc1s_nm wx_pc2s_nm /// 
wx_pc1b_nm_NMA wx_pc2b_nm_NMA wx_pc1c_nm_NMA wx_pc2c_nm_NMA wx_pc1f_nm_NMA wx_pc1h_nm_NMA wx_pc2h_nm_NMA /// 
wx_pc1n_nm_NMA wx_pc2n_nm_NMA wx_pc1s_nm_NMA wx_pc2s_nm_NMA


**** all spatial lags of capital stocks 
test wx_pc1b wx_pc2b wx_pc1c wx_pc2c wx_pc1f wx_pc1h  wx_pc2h wx_pc1n wx_pc2n wx_pc1s wx_pc2s ///
...
*** Coefficients of the capital stocks (and logs of the capital stocks) 
*** These (along with the coefficients and st. errors of $caps and all the wx_pc*) 
*** are the inputs for the Matlab code to calculate direct and indirect spatial effects 
